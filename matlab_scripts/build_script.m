...
%   6. compile via GCC → executable
    ...
    bridge_header_name = 'model_rt_bridge.h';
    ...
    try
        ...
        cmd = sprintf(['gcc -O2 -Wall -pthread ' ...
                       ...
                       '-o "%s"'], ...
            code_dir, c_core_src, bridge_header_name, ...
...
%PARSE_STRUCT_FIELDS Extract field names from a C struct definition
    ...
    % ERT annotates each declaration with a C comment.  Remove comments
    % before splitting at semicolons so a comment cannot hide the next field.
    inner = regexprep(inner, '/\*[\s\S]*?\*/', '');
