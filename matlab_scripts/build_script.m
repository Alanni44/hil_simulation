...
%   6. compile via GCC → executable
    ...
    types_h = fullfile(code_dir, [build_model '_types.h']);
    if ~exist(types_h, 'file')
        % Some ERT versions put types in the main header
        types_h = fullfile(code_dir, [build_model '.h']);
    end
    if ~exist(types_h, 'file')
        write_result(result_file, -1, 'Generated types header not found');
        return;
    end

    u_fields = parse_struct_fields(types_h, [build_model '_U_t']);
    y_fields = parse_struct_fields(types_h, [build_model '_Y_t']);
    ...
    if isfield(field_mapping, 'tunable_params')
        ...
        for ti = 1:length(tp_names)
            tn = tp_names{ti};
            if ~isKey(std_inputs, tn) && ~any(strcmp(std_input_names, tn))
    ...
    try
        ...
        cmd = sprintf(['gcc -O2 -Wall -pthread ' ...
                       ...
                       '-o "%s"'], ...
            code_dir, c_core_src, model_config_h, bridge_h, ...
