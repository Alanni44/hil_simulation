...
my_uav_model_U_t my_uav_model_U;
my_uav_model_Y_t my_uav_model_Y;
...
void my_uav_model_step(void) {
    ...
    /* --- manual (flight_mode=0): direct attitude control --- */
    if (my_uav_model_U.flight_mode == 0) {
        my_uav_model_Y.roll = my_uav_model_U.roll_cmd;
        my_uav_model_Y.pitch = my_uav_model_U.pitch_cmd;
        my_uav_model_Y.yaw = my_uav_model_U.yaw_cmd;

        double throttle = my_uav_model_U.throttle;
        if (throttle < 0.0) throttle = 0.0;
        if (throttle > 100.0) throttle = 100.0;
        double vz = (throttle - 50.0) * 0.1;  /* 50%=hover, ±5m/s range */
        double spd = 5.0;
        double dx = target_x - my_uav_model_Y.pos_x;
        double dy = target_y - my_uav_model_Y.pos_y;
        double dist = sqrt(dx*dx + dy*dy);
        my_uav_model_Y.vel_x = (dist > 0.5) ? (dx / dist) * spd : my_uav_model_U.pitch_cmd * 0.1;
        my_uav_model_Y.vel_y = (dist > 0.5) ? (dy / dist) * spd : my_uav_model_U.roll_cmd * 0.1;
        my_uav_model_Y.vel_z = vz;
    } else {
        /* auto/semi-auto: position-hold dynamics */
    ...
    if (my_uav_model_U.min_height > 0 && my_uav_model_Y.pos_z < my_uav_model_U.min_height)
        my_uav_model_Y.pos_z = my_uav_model_U.min_height;
    if (my_uav_model_U.max_height > 0 && my_uav_model_Y.pos_z > my_uav_model_U.max_height)
        my_uav_model_Y.pos_z = my_uav_model_U.max_height;
