...
cleanup() { kill $CORE_PID $PY_PID $UE4_PID 2>/dev/null; wait 2>/dev/null; echo "cleanup"; }
trap cleanup EXIT
...
check "generate_test_model" $(test -f "$SLX" || echo 1)
...
check "ERT build" $(test -f "$EXE" || echo 1)
...
if len(d)==S:
    v=struct.unpack(F,d)
    print('%.2f %.2f %.2f %d %d %d'%(v[4],v[5],v[6],v[34],v[35],v[33]))
s.close()
" 2>/dev/null; }
...
t5ok=1; [ -z "$(poll)" ] && t5ok=0; check "T5-tune_no_crash" $t5ok

echo -n "T6: "; send "land"; sleep 3; send "tune" '{"u_mass":-0.1}'; sleep 1
t6ok=1; [ -z "$(poll)" ] && t6ok=0; check "T6-neg_mass_no_crash" $t6ok
